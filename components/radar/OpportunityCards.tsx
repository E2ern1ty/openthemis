  ...
  if (!opportunities.length) {
    return (
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-slate-500">{t('舆情研判', 'Risk assessment')}</h3>
        <div className="card p-6 text-center">
          ...
          <p className="text-sm text-slate-500">{t('暂未识别到关键研判点', 'No key findings identified yet')}</p>
          <p className="text-xs text-slate-400 mt-1">{t('设置"本方主体"后重新分析，AI 将从舆情中提炼风险与应对洞察', 'Set "your subject" and re-analyze; AI will distill risk and response insights')}</p>
        </div>
      </div>
    );
  }
        ...
        >
          <RefButton
            reference={{
              ...
              label: `${t('研判', 'Finding')}${idx + 1}: ${opp.title}`,
          ...
          <div className="flex items-start justify-between mb-2 pr-8">
            <h4 className="text-sm font-bold text-slate-900 leading-snug">
              <span className="text-blue-600">{t(`研判 ${idx + 1}：`, `Finding ${idx + 1}: `)}</span>
              {opp.title}
            </h4>
          </div>
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-slate-500">{t('置信度', 'Confidence')}</span>
              <ConfidenceDots level={opp.confidence} />
            </div>
            <span className="text-[10px] text-slate-400">
              {t(`${opp.evidence.length} 条证据`, `${opp.evidence.length} evidence`)}
            </span>
          ...
          <div className="bg-slate-50 rounded-lg p-2.5 mb-3">
            <p className="text-[10px] text-slate-500 mb-1.5">{t('证据摘录：', 'Evidence: ')}</p>
          ...
          {opp.risk_note && (
            <div className="flex items-start gap-1.5 px-2.5 py-2 bg-amber-50 rounded-lg border border-amber-100">
